...
from utils import (
    DEFAULT_MAX_CHARS, brace_balanced, postprocess_latex,
    console, print_status, print_warning, print_error,
    extract_headings, deduplicate_paragraphs
)
...
def generate_chunk_latex(chunk_text: str, outline: str, previous_summary: str,
                          ...
                          source_tracing: bool = False, ocr_mode: str = "1",
                          available_packages: list = None,
                          custom_addons: list = None,
                          covered_topics: list = None) -> str:
    ...
    if covered_topics:
        system_prompt += (
            f"\n\n【已覆盖主题清单】以下主题已在前面章节生成，"
            f"如果当前片段涉及相同主题，请只补充新信息，不要重复展开已有内容："
...
# ---------- 跨片段查重（A 变体） ----------
def detect_cross_chunk_duplicates(chunk_outputs: list[str], fmt: str = "latex") -> list[dict]:
    """
    基于各 chunk 摘要检测跨片段的内容重复。
    返回建议列表，每个建议包含 duplicate_indices 和 action。
    """
    if not DEEPSEEK_API_KEY or len(chunk_outputs) < 2:
        return []

    summaries = []
    for i, output in enumerate(chunk_outputs, 1):
        headings = extract_headings(output, fmt)
        head = output[:250].replace('\n', ' ')
        tail = output[-250:].replace('\n', ' ')
        heading_str = "\n".join(f"  - {h}" for h in headings[:6])
        summaries.append(
            f"片段 {i}:\n标题列表:\n{heading_str}\n开头摘要: {head}\n结尾摘要: {tail}"
        )

    system_prompt = (
        "你是一位学术编辑。请检查以下长文档笔记的各片段摘要，"
        "识别是否存在不同片段对同一主题做了重复展开（内容冗余），而非必要的递进或回顾。"
        "\n\n判断标准："
        "\n1. 如果两个片段都包含相同的章节标题且内容高度相似，视为重复。"
        "\n2. 如果某片段几乎没有实质内容（只有标题框架），可建议精简。"
        "\n3. 只判断真正的内容重复，前后呼应、不同角度的论述是正常的，不算重复。"
        "\n\n输出要求："
        "\n- 输出必须为 JSON 数组，不要添加任何解释或 markdown 代码块。"
        '\n- 格式: [{"duplicate_indices": [2, 4], "reason": "...", "action": "merge"}]'
        "\n- action 只能是 merge 或 delete。"
        "\n- 如果没有重复，输出空数组 []。"
    )
    ...
    try:
        ...
        result = strip_fence_wrappers(result)
        import json
        suggestions = json.loads(result)
        if isinstance(suggestions, list):
            return suggestions
    except Exception as e:
        print_warning(f"跨片段查重检测失败：{e}")

    return []


def _apply_duplicate_suggestions(chunk_outputs: list[str], suggestions: list[dict], fmt: str = "latex") -> list[str]:
    """根据 LLM 的查重建议，本地执行合并/精简"""
    if not suggestions:
        return chunk_outputs

    result = list(chunk_outputs)

    for sug in suggestions:
        indices = sug.get("duplicate_indices", [])
        action = sug.get("action", "")
        if len(indices) < 2:
            continue

        primary_idx = indices[0] - 1  # 转为 0-based
        for dup_idx in indices[1:]:
            dup_idx -= 1
            if primary_idx < 0 or dup_idx >= len(result) or primary_idx >= len(result):
                continue

            if action == "delete":
                print_status(f"根据查重建议删除重复片段 {dup_idx + 1}", style="yellow")
                result[dup_idx] = ""
            else:  # merge: 对重复片段做严格段落级去重
                combined = result[primary_idx] + "\n\n" + result[dup_idx]
                deduped = deduplicate_paragraphs(combined, fmt=fmt, threshold=0.55, window=50)
                orig_len = len(result[primary_idx]) + len(result[dup_idx])
                if len(deduped) < orig_len * 0.75:
                    result[primary_idx] = deduped
                    result[dup_idx] = ""
                    print_status(f"片段 {dup_idx + 1} 已合并到片段 {primary_idx + 1}", style="yellow")
    ...
    if len(text) > max_chars:
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            if output:
                ...
                # B: 累积已覆盖主题
                new_topics = extract_headings(output, "latex")
                for t in new_topics:
        ...
        if len(chunk_outputs) >= 2:
            dup_suggestions = detect_cross_chunk_duplicates(chunk_outputs, fmt="latex")
            if dup_suggestions:
                chunk_outputs = _apply_duplicate_suggestions(chunk_outputs, dup_suggestions, fmt="latex")
        ...
        full_content = deduplicate_paragraphs(full_content, fmt="latex")
    ...
    if len(text) > max_chars:
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            if output:
                ...
                previous_summary = output[-500:] if len(output) > 500 else output
                # B: 累积已覆盖主题
                new_topics = extract_headings(output, "markdown")
                for t in new_topics:
                    if t not in covered_topics:
                        covered_topics.append(t)
            else:
        ...
        # A变体: 跨片段查重
        if len(chunk_outputs) >= 2:
            dup_suggestions = detect_cross_chunk_duplicates(chunk_outputs, fmt="markdown")
            if dup_suggestions:
                chunk_outputs = _apply_duplicate_suggestions(chunk_outputs, dup_suggestions, fmt="markdown")
        ...
        parts = [p for p in [intro] + chunk_outputs + [summary] if p.strip()]
        full_content = "\n\n".join(parts)
        full_content = deduplicate_paragraphs(full_content, fmt="markdown")
        print_status(f"分段生成完成（共 {len(chunk_outputs)} 段）。", style="green")
        return full_content, concept_map
