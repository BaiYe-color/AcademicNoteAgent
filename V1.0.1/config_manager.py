...
def update_usage_stats(config, selected_config):
    ...
    for key in ["cjk_serif", "cjk_sans", "cjk_mono", "en_serif", "en_sans", "en_mono",
                ...
                "ocr_mode", "note_style", "custom_style_name"]:
...
def get_personalized_preset(config):
    ...
    # 笔记风格
    style_stats = stats.get("note_style", {})
    if style_stats:
        best_style = max(style_stats, key=style_stats.get)
        preset["note_style"] = best_style if style_stats[best_style] >= 2 else "narrative"
    else:
        preset["note_style"] = "narrative"

    # 自定义风格
    csn_stats = stats.get("custom_style_name", {})
    if csn_stats:
        best_csn = max(csn_stats, key=csn_stats.get)
        preset["custom_style_name"] = best_csn if csn_stats[best_csn] >= 2 else None
    else:
        preset["custom_style_name"] = None

    # 如果预设包含自定义风格名，加载对应的 addons 和 base_style
    custom_styles = config.get("custom_styles", {})
    if preset.get("custom_style_name") and preset["custom_style_name"] in custom_styles:
        style_data = custom_styles[preset["custom_style_name"]]
        preset["custom_addons"] = style_data.get("addons", [])
        preset["note_style"] = style_data.get("base_style", "narrative")
    else:
        preset["custom_addons"] = []
