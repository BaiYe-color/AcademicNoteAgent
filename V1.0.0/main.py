...
def process_pdf(file_path, config, skip_reflect=False, skip_revise=False, resume_state=None, batch_mode=False, template_choice=None):
    if resume_state:
        ...
        if output_format == "markdown":
            ...
            print(f"[DEBUG] main.py received from generate_markdown_content: type={type(result)}, len={len(result) if result else 0}")
            ...
            print(f"[DEBUG] main.py received from generate_latex_content: type={type(result)}, len={len(result) if result else 0}")
            content, concept_map = result
        if not content:
