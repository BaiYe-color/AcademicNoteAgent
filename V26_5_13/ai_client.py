#!/usr/bin/env python3
"""DeepSeek AI 调用：LaTeX / Markdown 生成、修正、修订、学科识别"""
import sys
import openai
...
def generate_latex_content(text: str, api_key: str, max_chars: int = DEFAULT_MAX_CHARS, content_type: str = None, source_tracing: bool = False, ocr_mode: str = "1") -> str:
    if not api_key:
    ...
    if ocr_mode == "2":
        base_system += (
            "\n\n注意：部分页面的文字中包含“[OCR 补充提取]”标记，"
            "表示该部分内容是从图片或图表中识别出的补充文本。"
            "请将其合理融入正文，不要直接显示该标记。"
        )
    ...
    print_status("正在调用 AI 整理文本（LaTeX）...", style="magenta", icon="🤖")
    try:
        resp = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
...
def generate_markdown_content(text: str, api_key: str, max_chars: int = DEFAULT_MAX_CHARS, content_type: str = None, source_tracing: bool = False, ocr_mode: str = "1") -> str:
    if not api_key:
    ...
    system_md = (
        "你是一位学术笔记整理专家。请将用户提供的文本整理成结构化的 Markdown 笔记。"
        "要求：\n"
        "1. 使用 #、##、### 构建层级标题。\n"
        "2. 正文用自然段落呈现，用自己的话重述概念。\n"
        "3. 适当使用 **粗体**、*斜体*、列表（- 或 1.）、引用（>）等格式。\n"
        "4. 包含引言和总结段落（分别用 ## 引言 和 ## 总结 作为标题）。\n"
        "5. 如果启用了原文追溯（source_tracing=True），请在关键论点后添加 [原文第X页] 或 [Slide X] 标记。\n"
        "6. 只输出 Markdown 正文，不要包含任何前言或结语。"
    )
    ...
    print_status("正在调用 AI 生成 Markdown...", style="magenta", icon="📝")
    try:
        resp = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
...
def revise_content(current_content: str, user_comment: str, api_key: str, format: str = "latex", source_tracing: bool = False) -> str:
    if not api_key:
    ...
    system_prompt = (
        ...
        "不要添加任何解释。确保输出为纯{format_desc}，且所有现有格式标记（包括原文追溯标记）不得被删除。"
    )
    print_status(f"正在修订笔记（{format_desc}）...", style="magenta", icon="✏️")
    try:
        resp = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
